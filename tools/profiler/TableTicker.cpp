...
void TableTicker::doNativeBacktrace(ThreadProfile &aProfile, TickSample* aSample)
{
  ...
  for (uint32_t i = pseudoStack->stackSize(); i > 0; --i) {
    ...
    if (!entry.js() && /*FIXME*/ 0 == strcmp(entry.label(), "EnterJit")) {
      // Found JIT entry marker.  Unwind what we have, then restore the saved state.
      uint32_t *vSP = reinterpret_cast<uint32_t*>(entry.stackAddress());

      array.count += EHABIStackWalk(*mcontext, vSP, sp_array + array.count,
                                    pc_array + array.count, array.size - array.count);
  ...
  // Now unwind whatever's left.
  array.count += EHABIStackWalk(*mcontext, aProfile.GetStackTop(),
                                sp_array + array.count, pc_array + array.count,
